...
def get_sound_name(program_number):
    if (
        not isinstance(program_number, int)
        or program_number < 0
        or program_number > len(MIDI_SOUNDS) - 1
    ):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--midi",
        type=str,
        help="Path to the Midi file. (default: Uses first .mid in folder)",
    )
    parser.add_argument(
        "-b",
        "--bar-limit",
        type=int,
        default=0,
        help="The amount of bars to convert. 0 means no limit. (default: %(default)s)",
    )
    parser.add_argument(
        "-n",
        "--notes-per-bar",
        type=int,
        default=64,
        help="The resolution. Usually in steps of 4 (4, 8, 16...)."
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="How many spaces to use for indentation in the output. (default: %(default)s)",
    )
    parser.add_argument(
        "-g",
        "--guess-instrument",
        action="store_true",
        help="Use the sounds closest to the instruments mentioned in the MIDI. (default: off)",
    )
    parser.add_argument(
        "-f",
        "--flat-sequences",
        action="store_true",
        help="No complex timing or chords. (default: off)",
    )
    parser.add_argument(
        "-s",
        "--small-print",
        action="store_true",
        help="Put all notes on a single line."
...
def build_tracks(events, cycle_len, args):
    ...
    for track_key in sorted(events):
        ...
        num_cycles = min(
            (int(max_time / cycle_len) + 1),
            args.bar_limit if args.bar_limit > 0 else float("inf"),
        )
        bars = []
        ...
        for c in range(int(num_cycles)):
            ...
            bar = (
                get_flat_mode_bar(notes_in_cycle)
                if args.flat_sequences
                else get_poly_mode_bar(
                    notes_in_cycle, start, cycle_len, args.notes_per_bar
                )
            )
...
def get_poly_mode_bar(events, cycle_start, cycle_len, notes_per_bar):
    ...
    compressed = compress_sequence(simplified)

    return compressed[0] if len(compressed) == 1 else f"[{' '.join(compressed)}]"
